...
news_sites_uids =['el_universal', 'el_pais']
...
def _extract():
    ...
    for news_sites_uid in news_sites_uids:
        ...
        subprocess.run(['find', '.', '-name', '{}*'.format(news_sites_uid),
                       '-exec', 'mv', '{}', '../transform/{}_csv'.format(news_sites_uid),
                       ';'], cwd='./extract')
...
def _transform():
    ...
    for news_sites_uid in news_sites_uids:
        dirty_data_filename = '{}_csv'.format(news_sites_uid)
        ...
        subprocess.run(['mv', clean_data_filename, '../load{}.csv'.format(news_sites_uids)],
                       cwd='./transform')
